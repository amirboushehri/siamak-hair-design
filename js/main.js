...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Open/closed status dot based on Toronto time.
  ...
  function updateStatusDot() {
    ...
    const day = map.weekday; // Mon, Tue, Wed, Thu, Fri, Sat, Sun
    const hour = parseInt(map.hour, 10);
    const minute = parseInt(map.minute, 10);
    const mins = hour * 60 + minute;
    let open = false;
    if (['Tue', 'Wed', 'Thu', 'Fri'].includes(day)) open = mins >= 600 && mins < 1080;
    else if (day === 'Sat') open = mins >= 540 && mins < 1080;
    dot.classList.toggle('closed', !open);
    dot.setAttribute('aria-label', open ? 'Open now' : 'Closed');
  }
  updateStatusDot();
  ...
  if (navbarEl && stackEl) {
    ...
    const onScroll = () => {
      ...
      navbarEl.classList.toggle('scrolled', y > solidThreshold());
