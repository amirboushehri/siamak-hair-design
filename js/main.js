...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateStatus() {
    ...
    const statusPill = document.querySelector('.btn-status');
    if (statusPill) {
    ...
    document.querySelectorAll('a.btn[href="#book"]').forEach((el) => {
      // Skip the "Schedule your next appointment" nav button (own copy)
      // and the status pill (has nested spans handled above).
      if (el.classList.contains('nav-book-desktop')) return;
      if (el.classList.contains('btn-status')) return;
      el.textContent = isOpen ? 'Book' : 'Schedule now';
    });
  }
